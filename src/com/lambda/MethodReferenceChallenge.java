...
public class MethodReferenceChallenge {
    ...
    public static void main(String[] args) {
        ...
        String[] names = {"Anna", "Bob", "Francis", "Eva", "Cameron"};
        Person tim = new Person("Tim");
        List<UnaryOperator<String>> list = new ArrayList<>(List.of(
                String::toUpperCase,
                s -> s += " " + getRandomChar('D', 'M') + ".",
                s -> s += " " + getReverseString(s, 0, s.indexOf(" ")),
...
//                s -> new String("Howdy "+s),
    ...
    private static String getReverseString(String s, int start, int end) {
        return new StringBuilder(s.substring(start, end + 1)).reverse().toString();

    }

    private static String reverse(String s) {
        return getReverseString(s, 0, s.length() - 1);
    }
}
